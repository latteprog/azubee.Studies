...
def test_improvement_abs_skill(trained, untrained, is_graph_norm, norm_val=0.05):
    ...
    trained_improvements = (
        trained.groupby(["User", "ExerciseSkill"])
        .mean()["ImprovementAbsNormalizedScores"]
        .to_numpy()
    )
    untrained_improvements = (
        untrained.groupby(["User", "ExerciseSkill"])
        .mean()["ImprovementAbsNormalizedScores"]
        .to_numpy()
    )
    ...
    return t_test_result, calculate_cohends_d(
        trained_improvements, untrained_improvements
    )
...
improv_t_res, improv_cohens = test_improvement_abs_skill(
    trained, untrained, is_graph_norm=False, norm_val=0.05
)
improv_exercise_t_res, improv_exercise_cohens = test_improvement_abs_exercise(
    trained, untrained, is_graph_norm=True, norm_val=0.05
)
normalized_t_res, normalized_cohens = test_improvement_normalized_change_skill(
    trained, untrained, is_graph_norm=True, norm_val=0.05
)
(
    normalized_exercise_t_res,
...
data = pd.DataFrame(
    {
        "type": [
            "improvement_abs_skill",
            "improvement_abs_exercise",
            "normalized_change_skill",
            "normalized_change_exercise",
        ],
        "t": [
            improv_t_res.statistic,
            improv_exercise_t_res.statistic,
            normalized_t_res.statistic,
            normalized_exercise_t_res.statistic,
        ],
        "p": [
            improv_t_res.pvalue,
            improv_exercise_t_res.pvalue,
            normalized_t_res.pvalue,
            normalized_exercise_t_res.pvalue,
        ],
        "cohens": [
            improv_cohens,
            improv_exercise_cohens,
            normalized_cohens,
            normalized_exercise_cohens,
        ],
    }
